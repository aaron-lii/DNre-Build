...
def create_out_ability():
    with gr.Tab("战力"):
        ...
        with gr.Row():
            ...
            dps_increase_plot = gr.BarPlot(x="属性", y="收益率", title="三属性纹章增量和石板替换攻击收益率",
                                           sort="-y", height=300, x_label_angle=45)
        with gr.Row():
            def_text = gr.TextArea("神操，不会中技能！", label="当前配装的物防生存力", lines=2)
            def_increase_plot = gr.BarPlot(x="属性", y="收益率", title="三属性纹章增量和石板替换物防收益率",
                                           sort="-y", height=300, x_label_angle=45)
        with gr.Row():
            magic_def_text = gr.TextArea("神操，不会中技能！", label="当前配装的魔防生存力", lines=2)
            magic_def_increase_plot = gr.BarPlot(x="属性", y="收益率", title="三属性纹章增量和石板替换魔防收益率",
                                                 sort="-y", height=300, x_label_angle=45)
        with gr.Row():
            gr.Markdown("\n\n\n\n\n\n")
